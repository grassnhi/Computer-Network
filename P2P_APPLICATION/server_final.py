        ...
        try:
            ...
            if data.startswith("publish"):
                ...
                while os.path.exists(file_path):
                    # If the file exists, modify the fname to make it unique
                    fname = f"{fname}({counter})"
                    file_path = os.path.join(public_path, f"{ip}_published.txt")
                    counter += 1

                with open(file_path, "a") as file:
                    file.write(f"{lname+fname}\n")
            ...
            if data.startswith("fetch"):
                ...
                try:
                    _, fname = data.split("_", 1)  # Limit the split to 1 part
                except ValueError:
                    print("Error: Invalid data format.")
...
def check_name_published(fname):
